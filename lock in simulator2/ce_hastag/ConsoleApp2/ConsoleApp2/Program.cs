...
class Program
{
    ...
    public static List<((int, int), (int, int))> swappedPairs = new List<((int, int), (int, int))>();

    public static int pairsSwappedThisTable = 0;

    public static string optimiseTable(string table)
    {
        if (pairsSwappedThisTable == 2016)
        {
            Console.WriteLine("cant get any better this time");
            Environment.Exit(0);
        }

        Random random = new Random();

        //char[] array = table.ToCharArray();

        char[,] array = new char[table.Length, table.Length];

        for (int i = 0; i < table.Length; i++)
        {
            char currentChar = table[i];
        ...
        int lx = random.Next(0, 8);
        int rx = random.Next(0, 8);
        int ly = random.Next(0, 8);
        int ry = random.Next(0, 8);

        while (swappedPairs.Contains(((lx, ly), (rx, ry))))
        {
            lx = random.Next(0, 8);
            rx = random.Next(0, 8);
            ly = random.Next(0, 8);
            ...
            pairsSwappedThisTable++;


        }

        swappedPairs.Add(((lx, ly), (rx, ry)));

        char temp = array[lx, ly];
        array[lx, ly] = array[rx, ry];
        array[rx, ry] = temp;
        ...
        string optimized = string.Join("", tempArray);

        return optimized;
    }

    public static int EvaluateTable(string table, string textToType)
    {
        int tableWidth = 8;
        ...
        foreach (char currentChar in textToType)
        {
            if (!charPosMap.ContainsKey(currentChar))
            {
                continue;
            ...
            {
                int lowestCostToCurrentChar = int.MaxValue;
                ...
                foreach (var prevPos in possibleCurrentPos)
                {
                    ...
                    if (newCost < lowestCostToCurrentChar)
                    {
                        lowestCostToCurrentChar = newCost;
                    }
                }
                possibleNewPos.Add((newRow, newColumn), lowestCostToCurrentChar);
            }
    ...
    public static string SwapChars(string table, int index1, int index2)
    {
    ...
    public static string ReplaceChars(string table, int index, char newChar)
    {
        char[] arr = table.ToCharArray();
        arr[index] = newChar;
        return new string(arr);
    ...
    public static int getTotalClicks()
    {
        ...
        //string inputTableString = "FFmlIN RWQsd aVMU.uinteqAEboem ugLSalusib.h ctronOjCuevDPHrTwafyJ"; //record
        //FmbINQRWuld aVMUCunit qAEsoemregLSalusib.h et onOj.uavDTHrPwcfyJ:74140

        string inputTableString = "FmbINQRWuld aVMUCunit qAEsoemregLSalusib.h et onOj.uavDTHrPwcfyJ";
        ...
        while (minCost > 70000)
        {
            string oldInputTableString = inputTableString;

            int tableWidth = 8;
            int tableHeight = 8;

            //inputTableString = " RfPTU ySvoqwJAhniagENcu emVIdltsbQWfpr.LO FCOH i"; // invalid
            //inputTableString = " EMySA TcldnjFDvatifKJue sbWDQhmoqPCwIr. VGWHNRXe"; // invalid
            //inputTableString = " EMCSA .cldnjFDvatiPg Jue sbWDQhmoqfTwIr. V WHNRe"; // 82503
            //inputTableString = "FLPRfQODNr m.EVgeqtoSH sna WCbilucAj.p dvwJUMhiTy"; // 80661
            //inputTableString = " WACDEh g.    . U rusm FT lfje HS DYRO IR tvwa JQ pnic L .    .b qPONMg "; // 93881 (8x7)
            //inputTableString = "aWACDEhsg.    .iU rusm FT lfje HS dyLo IR tvwa JQ pnic rt.    .boQPONMge"; // 104431 (8x7)
            //inputTableString = "FLPRNQODof s.EVreqtgSH mna WCbilucAj.p dvwJUMhiTy"; // 83000

            //inputTableString = randomTable([" ", " ", " ", " ", "e", "e", "e", "i", "u", "t", "s", "i", "u", "t", "s", "a", "s", "i", "u", "t", "s", "a", "n", "l", "a", "n", "l", "r", "m", "o", "c", "d", "p", ".", ".", "b", "g", "q", "v", "h", "N", "C", "V", "P", "f", "S", "A", "I", "M", "j", "D", "L", "Q", "E", "w", "T", "U", "F", "O", "y", "J", "R", "W", "H"]);

            //inputTableString = randomTable("RFNmhIQWELdna.MUCo tipqAuruemsegcnpluribVa et oSOj.uavDTHwPrlfyJ".ToCharArray());

            //inputTableString = "RFNmhIQWELdna.MUCu ti qAusoemregcnalusibVb et oSOj.uavDTHwPrlfyJ"; //75520

            //FmhINQRWELdna.MUCu ti qAusoemregcnalusibVb et oSOj.uavDTHwPrlfyJ // 75128

            //inputTableString = "RFNmhIQWELdna.MUCu ti qAusoemregcnalusibVb et oSOj.uavDTHwPrlfyJ";

            //inputTableString = "RFNmhIQWELdna.MUCo tipqAuruemsegcnpluribVa et oSOj.uavDTHwPrlfyJ";

            inputTableString = optimiseTable(oldInputTableString);
            ...
            foreach (var pos in possibleCurrentPos)
            {
                //Console.WriteLine(inputTableString + ":" + pos.Value);
                ...
                if (pos.Value < minCost)
                {
                    ...
                    minCost = pos.Value;
                    oldInputTableString = inputTableString;
                }
            }

            if (minCost < 75000)
            {
                //bestCandidates.Add(minCost);
                Console.WriteLine(inputTableString + ":" + minCost);
                //record = minCost;
                swappedPairs = new List<((int, int), (int, int))>();
                //Thread.Sleep(3000);
            }
        ...
        //Console.WriteLine(inputTableString);

        return minCost;
    }

    static string ScrambleTable(string table, int swapsCount)
    {
        Random rnd = new Random();
        string result = table;
        for (int i = 0; i < swapsCount; i++)
        {
            int p1 = rnd.Next(0, 64);
            int p2 = rnd.Next(0, 64);
            result = SwapChars(result, p1, p2);
        }
        return result;
    }
    ...
    static void Main(string[] args)
    {
        ...
        string bestTable = randomTable([
            ' ', ' ', ' ', ' ', 'e', 'e', 'e', 'i', 'u', 't', 's', 'i', 'u', 't', 's', 'a', 's', 'i', 'u', 't', 's',
            'a', 'n', 'l', 'a', 'n', 'l', 'r', 'm', 'o', 'c', 'd', 'p', '.', '.', 'b', 'g', 'q', 'v', 'h', 'N', 'C',
            'V', 'P', 'f', 'S', 'A', 'I', 'M', 'j', 'D', 'L', 'Q', 'E', 'w', 'T', 'U', 'F', 'O', 'y', 'J', 'R', 'W', 'H'
        ]);
        int bestCost = EvaluateTable(bestTable, inputString);

        while (bestCost > 90000)
        {
            bestTable = randomTable([
                ' ', ' ', ' ', ' ', 'e', 'e', 'e', 'i', 'u', 't', 's', 'i', 'u', 't', 's', 'a', 's', 'i', 'u', 't', 's',
                'a', 'n', 'l', 'a', 'n', 'l', 'r', 'm', 'o', 'c', 'd', 'p', '.', '.', 'b', 'g', 'q', 'v', 'h', 'N', 'C',
                'V', 'P', 'f', 'S', 'A', 'I', 'M', 'j', 'D', 'L', 'Q', 'E', 'w', 'T', 'U', 'F', 'O', 'y', 'J', 'R', 'W', 'H'
            ]);
            bestCost = EvaluateTable(bestTable, inputString);
        }
        */

        bool improved = true;

        int globalBestCost = EvaluateTable(bestTable, inputString);
        string globalBestTable = bestTable;

        int restarts = 0;
        int maxRestarts = 15;

        Console.WriteLine($"Štartujeme. Skóre: {bestCost}");

        while (restarts < maxRestarts + 1)
        {
        FoundBetterWithReplace:;

            // ----------------------------------------------------
            // FÁZA 1: Prehadzovanie
            // ----------------------------------------------------
            improved = true;
            while (improved)
            {
                ...
                for (int i = 0; i < 63; i++)
                {
                    ...
                    {
                        string testTable = SwapChars(bestTable, i, j);
                        int currentCost = EvaluateTable(testTable, inputString);
                        ...
                        if (currentCost < bestCost)
                        {
                            ...
                            Console.WriteLine($"[SWAP] Nové najlepšie: {bestCost} kliknutí | Tabuľka: {bestTable}");
                            goto FoundBetterSwap;
                        }
                    }
                }
            FoundBetterSwap:;
            ...
            char[] charsNeeded =
            [
                ' ', '.', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'y', 'A', 'C', 'D', 'E', 'F', 'H', 'I', 'J', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W'
            ];
            ...
            while (improved)
            {
                ...
                for (int i = 0; i < 64; i++)
                {
                    for (int j = 0; j < charsAvailable.Length; j++)
                    {
                        if (bestTable[i] == charsAvailable[j]) continue;

                        string testTable = ReplaceChars(bestTable, i, charsAvailable[j]);

                        bool isValid = true;
                        for (int k = 0; k < charsNeeded.Length; k++)
                        {
                        ...
                        if (isValid)
                        {
                            ...
                            if (currentCost < bestCost)
                            {
                                ...
                                Console.WriteLine($"[REPLACE] Nové najlepšie: {bestCost} kliknutí | Tabuľka: {bestTable}");

                                goto FoundBetterWithReplace;
                            }
                        }
                    }
                }
            ...
            */
            ...
            if (bestCost < globalBestCost)
            {
                ...
                globalBestTable = bestTable;
                Console.WriteLine($"\nNOVÉ GLOBÁLNE MINIMUM: {globalBestCost}\n");
                restarts = 0;
            }
            else
            {
                restarts++;
                Console.WriteLine($"Zemetrasenie č. {restarts}...");
            }

            bestTable = ScrambleTable(globalBestTable, 4);
            bestCost = EvaluateTable(bestTable, inputString);
        }

        Console.WriteLine("Najlepšia tabuľka pre toto spustenie - " + globalBestTable + ":" + globalBestCost);
    }
}